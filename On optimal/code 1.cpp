...
vector<Node *> algorithm1(SFCR *sfc, Cluster *cluster, vector<Node *> &nodes, vector<Link *> &links, vector<VNF *> &VNFS)
{
    ...
    for (Node *node : nodes)
    {
        if (node->type == NodeType::LEO)
        {
            int choices[] = {3, 5};
            std::random_device rd;
            std::mt19937 gen(rd());
            std::uniform_int_distribution<> dis(0, 1);
            int nSatellites = choices[dis(gen)];
            double distance = sqrt(pow(node->position.x - sfc->sourceNode->position.x, 2) + pow(node->position.y - sfc->sourceNode->position.y, 2) + pow(node->position.z - sfc->sourceNode->position.z, 2));
            double time_period = 2 * M_PI * (EARTH_RADIUS + node->height) * 1000 / node->velocity;
            // cout<<"ratio: "<<ratio<<endl;
            // cout << "distance = " << distance << " comm range: " << node->communicationRange << endl;
            double visibility_window = time_period / nSatellites;
...
bool algorithm2(SFCR *sfcr, Cluster *cluster, vector<Node *> &available_satellites, vector<Link *> &links, vector<Graph> &graphs, int K)
{
    ...
    for (Node *node : available_satellites)
    {
        ...
        if (node->type == NodeType::LEO)
        {
            ...
            double distance = sqrt(pow(node->position.x - sfcr->sourceNode->position.x, 2) + pow(node->position.y - sfcr->sourceNode->position.y, 2) + pow(node->position.z - sfcr->sourceNode->position.z, 2));
    ...
    // Step 1: find the available intersatellite links

    for (Node *n1 : available_satellites)
    {
        for (Node *n2 : available_satellites)
        {
            Link *link = find_link(links, n1, n2);
            if (link)
            {
                double distance = sqrt(pow(n1->position.x - n2->position.x, 2) + pow(n1->position.y - n2->position.y, 2) + pow(n1->position.z - n2->position.z, 2));
                if (distance <= n2->communicationRange && distance <= n1->communicationRange)
                {
                    link->active = true;
                    link->revLink->active = true;
    ...
    std::priority_queue<std::pair<double, std::vector<Node *>>, std::vector<std::pair<double, std::vector<Node *>>>, Compare> kShortestPaths = kShortestPath(shortestNode, totalAvailableLinks, numberOfVNFs, numberOfVNFs * 2, graphs, K);
